...
@function_tool
async def set_timer(duration_text: str) -> str:
    """
    Set a timer in the macOS Clock app using natural language input with hours and/or minutes.

    Args:
        duration_text (str): Duration for the timer in natural language. Examples:
                             "an hour", "an hour and 30 minutes", "2 hours", "45 minutes"
    ...
    try:
        ...
        # Handle "an hour" as 1 hour
        duration_text_lower = duration_text_lower.replace("an hour", "1 hour")
        
        # Match hours and minutes
        hours_match = re.search(r'(\d+)\s*hour', duration_text_lower)
        minutes_match = re.search(r'(\d+)\s*minute', duration_text_lower)
        ...
        if total_minutes == 0:
            return "❌ Couldn't understand the duration. Please specify hours and/or minutes."
        
        # Format for Clock app mm:ss
        duration = f"{total_minutes}:00"
        
        # AppleScript using keystrokes to reliably input duration and start timer
        applescript = f'''
        tell application "Clock" to activate
        delay 1
        tell application "System Events"
            tell process "Clock"
                click button "Timer" of toolbar 1
                delay 0.5
